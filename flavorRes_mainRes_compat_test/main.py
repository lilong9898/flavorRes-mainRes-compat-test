...
for diff in repo.index.diff("HEAD"):
    if(diff.change_type == "M"):
        realResStagedFilePath = os.path.realpath(os.path.join(GIT_REPO_REAL_PATH, diff.b_path));
        for resSubDirPath in RES_SUB_DIR_PATHS:
            if(realResStagedFilePath.startswith(os.path.join(DIR_MAIN_RES_REAL_PATH, resSubDirPath))):
                listStrStagedFileRealPathsInMainRes.append(realResStagedFilePath);
            if(realResStagedFilePath.startswith(os.path.join(DIR_PRODUCT_FLAVOR_RES_REAL_PATH, resSubDirPath))):
                listStrStagedFileRealPathsInProductFlavorRes.append(realResStagedFilePath);

# 获取所有productFlavor/res文件夹下的文件路径
for (path, d, filelist) in os.walk(DIR_PRODUCT_FLAVOR_RES_REAL_PATH):
...
for strStagedFileRealPathsInMainRes in listStrStagedFileRealPathsInMainRes:
    if strStagedFileRealPathsInMainRes.replace(DIR_MAIN_RES_REAL_PATH, DIR_PRODUCT_FLAVOR_RES_REAL_PATH) in listStrFileRealPathsInProductFlavorRes and strStagedFileRealPathsInMainRes.replace(DIR_MAIN_RES_REAL_PATH, DIR_PRODUCT_FLAVOR_RES_REAL_PATH) not in listStrStagedFileRealPathsInProductFlavorRes:
...
print("检测的文件夹包括：%s"%(",".join(RES_SUB_DIR_PATHS)));
print("这些文件在main的res下有进入暂存区的修改，在%s的res下存在，而且没有进入暂存区的修改:"%(PRODUCT_FLAVOR));
print();
...
print();
print("这会导致%s/res不能正确反映main/res下的修改!"%(PRODUCT_FLAVOR));
print("==========================================================================================");
print();
